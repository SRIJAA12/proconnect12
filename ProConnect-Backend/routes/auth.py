...
def derive_student_name(email):
    """Create a readable fallback name from an email address."""
...
def student_register():
    ...
    Stores credentials in student_credentials collection and creates a visible
    student stub so the faculty dashboard can show the registered student before
    ...
    try:
        ...
        if not roll_number or not email or not password:
            return jsonify({'error': 'rollNumber, email and password are required'}), 400
        ...
        # Check if credentials already exist for this roll number OR email
        existing = creds_col.find_one({
            '$or': [
        ...
        # Store ONLY credentials — no student document created yet
        password_hash = bcrypt.hashpw(password.encode('utf-8'), bcrypt.gensalt())
        result = creds_col.insert_one({
        ...
        # Create a dashboard-visible stub in the students collection so faculty
        # can see the registered student even before the full form is submitted.
        students_col = collections['students']
        display_name = derive_student_name(email) or roll_number
        ...
        stub_doc = stub.to_dict()
        stub_doc['updated_at'] = datetime.utcnow()
        if existing_student:
            students_col.update_one({'_id': existing_student['_id']}, {'$set': stub_doc})
        else:
            students_col.insert_one(stub_doc)

        return jsonify({'message': 'Registration successful', 'student_id': str(result.inserted_id)}), 201

    except Exception as e:
        print(f"Error in student_register: {e}")
...
def student_login():
    """
    Student login — authenticates against student_credentials collection.
    ...
    try:
        ...
        # Find credential record by email
        credential = creds_col.find_one({
            'email': {'$regex': f'^{email}$', '$options': 'i'}
        })

        if not credential:
            return jsonify({'error': 'No student account found. Please sign up first.'}), 404

        if not bcrypt.checkpw(password.encode('utf-8'), credential['password_hash']):
            return jsonify({'error': 'Incorrect password. Please try again.'}), 401

        roll_number = credential.get('roll_number', '')

        # Update last login timestamp
        creds_col.update_one({'_id': credential['_id']}, {'$set': {'last_login': datetime.utcnow()}})
...
def student_forgot_password():
    try:
        ...
        print(f"Searching for student credentials: email={email}, roll_number={roll_number}")

        # Look up in student_credentials (where auth data lives)
        credential = creds_col.find_one({
            'roll_number': {'$regex': f'^{roll_number}$', '$options': 'i'},
            'email': {'$regex': f'^{email}$', '$options': 'i'}
        })
        ...
        if not credential:
            print(f"No credentials found for roll_number={roll_number} AND email={email}")
            return jsonify({'error': 'No matching student account found'}), 404
        ...
        creds_col.update_one(
            {'_id': credential['_id']},
            {'$set': {'student_reset_otp': otp, 'student_otp_expiry': otp_expiry}}
        )

        try:
            print(f"Sending student OTP to {email}...")
            _send_reset_otp_email(email, 'Student', otp, 'ProConnect - Student Password Reset OTP')
            print(f"Student OTP sent to {email}")
            return jsonify({'message': 'OTP sent successfully to your email.'}), 200
        except Exception as email_error:
            print(f"Failed to send student email: {str(email_error)}")
            return jsonify({'error': f'Unable to send reset email: {str(email_error)}'}), 500
...
def student_verify_otp():
    try:
        ...
        # Look up in student_credentials
        credential = creds_col.find_one({
            'roll_number': {'$regex': f'^{roll_number}$', '$options': 'i'},
            'email': {'$regex': f'^{email}$', '$options': 'i'}
        })
        ...
        stored_otp = credential.get('student_reset_otp')
        otp_expiry = credential.get('student_otp_expiry')
